...
### v3.4 - 飞行模式换IP (2026-03-16)
**无代理时通过飞行模式免费换IP：**

**流程：**
1. 频控触发 → 有代理则自动切换代理
2. 无代理 → 震动3次提醒 + 日志提示"请开关飞行模式"
3. 用户下拉控制中心 → 开飞行模式 → 关飞行模式（运营商分配新IP）
4. NetworkObserver 检测到网络恢复 → 等2秒稳定 → 自动清缓存+重建WebView+自动登录+断点续传

**技术实现：**
- 新增 `waitingForAirplaneToggle` 状态标记
- `onChange(of: networkObserver.isConnected)` 监听网络恢复
- 网络恢复后自动调用 `clearWebsiteDataAndRebuild()`
- 合并 `session_reset` 和 `switch_proxy` 处理逻辑
